...
for i in range(21):
    ...
    values += values[:1]
    ax.plot(angles, values, linewidth=2, linestyle='solid', label=project_name_list[i])
    ax.fill(angles, values, facecolor='blue', alpha=0.25)
    plt.legend(loc='upper right', bbox_to_anchor=(0.1, 0.1))
    plt.rcParams["figure.figsize"] = (60, 10)
    plt.savefig("Spider graph " + str(data.values[i][0]), bbox_inches='tight')
    fig = plt.figure()
    plt.figure().clear()
    plt.close()
    plt.cla()
...
values += values[:1]
ax.plot(angles, values, linewidth=2, linestyle='solid', label="group C")"""
